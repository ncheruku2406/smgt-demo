...
def do_consolidation(labeling_job_arn, payload, label_attribute_name, output_config_url):
    ...
    if "s3Uri" in payload:
        ...
        bucket, object_key = split_s3_path(s3_ref)
        payload = json.loads(s3_client.get_object(Bucket=bucket, Key=object_key).get('Body').read().decode('utf-8'))
    ...
    for p in range(len(payload)):
        ...
        try:
            ...
            dynamodb_data['source'] = payload[p]['dataObject']['content']
            ...
            for i in range(len(annotations)):
                ...
                dynamodb_data['annotation_content'] = annotation_content
            ...
            dynamodb_data['workerid'] = str(random.randint(0,9))
            dynamodb_response = dynamodb_table.put_item(Item=dynamodb_data)
